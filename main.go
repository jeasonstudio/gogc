...
func main() {
	fmt.Println("Let's Start Garbage Collector")
	// turn off golang's default garbage collector
...
func printMemStatsInfo(rms *runtime.MemStats) {
	fmt.Println("已申请且仍在使用的字节数: ", rms.Alloc)
	fmt.Println("          申请内存的次数: ", rms.Mallocs)
	fmt.Println("               GC元数据: ", rms.GCSys)
	fmt.Println("          引导程序的堆栈: ", rms.StackInuse)
	fmt.Println("        系统分配的堆内存: ", rms.HeapSys)
	fmt.Println("        具体内存分配情况: ", rms.BySize)
}
